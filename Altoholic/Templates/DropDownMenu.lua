...
-- *** AltoDropDownMenuButton ***

local function _OnClick(frame)
...
local function _GetChecked(frame)
	return frame.Check:IsShown()
end

local function _GetName(frame)
	return frame.NormalText:GetText()
end

local function _GetButtonWidth(frame)
	local width
...
addon:RegisterClassExtensions("AltoDropDownMenuButton", {
	OnClick = _OnClick,
...
local function _StartCounting(frame)
...
local function _StopCounting(frame)
	if frame.parent then
...
local function _DisableButton(frame, level, buttonIndex)
...
local function _EnableButton(frame, level, buttonIndex)
	local button = GetListButton(level, buttonIndex)
	if button then
		button:Enable()
	end
...
local function _SetButtonText(frame, level, buttonIndex, text, colorCode)
	local button = GetListButton(level, buttonIndex)
...
local function _SetButtonNotClickable(frame, level, buttonIndex)
	local button = GetListButton(level, buttonIndex)
	if button then
...
local function _SetButtonClickable(frame, level, buttonIndex)
	local button = GetListButton(level, buttonIndex)
	if button then
...
local function _HideDropDownMenu(frame, level)
...
local function _Close(frame, level)
	CloseLists(level)
end

local function _GetMaxButtonWidth(frame)
	local maxWidth = 0
...
local function _OpenColorPicker(frame, button)
	CloseMenus()
...
local function _RefreshDropDownSize(frame)
...
local function _GetListFromPool(frame, level)
	return listsPool[level]
end

local function _Toggle(frame, level, value, dropDownFrame, anchorName, xOffset, yOffset, menuList, button, autoHideDelay)
	level = level or 1
...
addon:RegisterClassExtensions("AltoDropDownList", {
	StartCounting = _StartCounting,
	StopCounting = _StopCounting,
	DisableButton = _DisableButton,
	EnableButton = _EnableButton,
	SetButtonText = _SetButtonText,
	SetButtonNotClickable = _SetButtonNotClickable,
	SetButtonClickable = _SetButtonClickable,
	HideDropDownMenu = _HideDropDownMenu,
	OnHide = _OnHide,
	Close = _Close,
	GetMaxButtonWidth = _GetMaxButtonWidth,
	OpenColorPicker = _OpenColorPicker,
	RefreshDropDownSize = _RefreshDropDownSize,
	GetListFromPool = _GetListFromPool,
	Toggle = _Toggle,
})
...
local function _Initialize(frame, initFunction, displayMode, level, menuList)
...
local function _Toggle(frame, anchor, xOffset, yOffset)
	local listFrame = listsPool[1]
	listFrame:Toggle(1, nil, frame, anchor, xOffset, yOffset)
end

local function _Refresh(frame, useValue, level)
...
local function _ClearAll(frame)
...
local function _CreateInfo()
	-- always work with the same table, return it after it is wiped
	return wipe(info)
end

local function _AddButtonInfo(frame, info, level)
	level = level or 1
...
local function _AddButton(frame, text, value, func, icon, isChecked, level)
	local info = _CreateInfo()
...
local function _AddButtonWithArgs(frame, text, value, func, arg1, arg2, isChecked)
	local info = _CreateInfo()
...
local function _AddCategoryButton(frame, text, value, level)
	local info = _CreateInfo()
...
local function _AddCloseMenu(frame)
	local info = _CreateInfo()
...
local function _AddTitle(frame, text, icon)
	local info = _CreateInfo()
...
local function _SetMenuWidth(frame, width, padding)
...
local function _SetButtonWidth(frame, width)
	if width == "TEXT" then
...
local function _GetText(frame)
	return frame.Text:GetText()
end

local function _JustifyText(frame, justification)
...
local function _SetSelectedName(frame, name, useValue)
...
local function _GetSelectedName(frame)
	return frame.selectedName
end

local function _SetSelectedValue(frame, value, useValue)
...
local function _GetSelectedValue(frame)
	return frame.selectedValue
end

local function _SetSelectedID(frame, id, useValue)
	frame.selectedID = id
	frame.selectedName = nil
	frame.selectedValue = nil
	frame:Refresh(useValue)
...
local function _GetSelectedID(frame)
...
local function _GetCurrentOpenMenuValue(frame)
	return currentOpenMenuValue
end

addon:RegisterClassExtensions("AltoDropDownMenu", {
	...
	AddButtonInfo = _AddButtonInfo,
	AddButton = _AddButton,
	AddButtonWithArgs = _AddButtonWithArgs,
	AddCategoryButton = _AddCategoryButton,
	AddCloseMenu = _AddCloseMenu,
	AddTitle = _AddTitle,
	Close = _Close,
	DisableDropDown = _DisableDropDown,
	EnableDropDown = _EnableDropDown,
	IsEnabled = _IsEnabled,
	SetAnchor = _SetAnchor,
	SetMenuWidth = _SetMenuWidth,
	SetButtonWidth = _SetButtonWidth,
	SetText = _SetText,
	GetText = _GetText,
	...
	GetCurrentOpenMenuValue = _GetCurrentOpenMenuValue,
})
